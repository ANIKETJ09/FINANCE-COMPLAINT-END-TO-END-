...
def get_model(show_summary=True):
    ...
    input_features = []
    for key  in schema.one_hot_features+schema.binary_features:
        input_features.append(
            tf.keras.Input(shape=(1,), name=CensusDatasetSchema.transformed_name(key))
        )
    
    input_features.append(
            tf.keras.Input(shape=(1,), name=CensusDatasetSchema.transformed_name(schema.text_feature))
        )

    initial_layer = tf.keras.layers.concatenate(input_features)
    layer_1 = tf.keras.layers.Dense(16, activation="relu")(initial_layer)
...
def _get_serve_tf_examples_fn(model, tf_transform_output):
    ...
    def serve_tf_examples_fn(serialized_tf_examples):
        ...
        feature_spec.pop(schema.target_column)
...
def _input_fn(file_pattern, tf_transform_output, batch_size=64):
    ...
    dataset = tf.data.experimental.make_batched_features_dataset(
        ...
        label_key=CensusDatasetSchema.transformed_name(schema.target_column),
    )
